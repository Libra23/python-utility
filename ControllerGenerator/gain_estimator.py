...
class gain_estimator:
    ...
    def eval_func(self, rho, u0, y0, Td):
        invC = self.ctrl_func(rho) ** -1
        e_pseudo, _, _ = control.lsim(invC, u0.flatten())
        r_pseudo = e_pseudo + y0
        y_pseudo, _, _ = control.lsim(Td, r_pseudo.flatten())
        return np.linalg.norm(y0 - y_pseudo)**2

    def estimate(self, rho0, input, output, Td):
